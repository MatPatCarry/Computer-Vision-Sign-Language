...
function createMessagePopup(msg) {
    ...
    condWarningElement.textContent = msg;
...
async function fetchData(file) {
    ...
    try {
        ...
        if (responseData) {
            ...
            if (!responseData.prob) {
                    
                const msg = `Incorrect result to perform autotyping`;
                createMessagePopup(msg);

                messageTimeout = setTimeout(() => {

                    console.log(`Popup closed after 2 seconds`);
                    window.close();
            ...
            else if (responseData.prob <= 0.9) {

                const msg = `To low probability of prediction to perform autotyping: ${responseData.prob}`;
                createMessagePopup(msg);
